...
export default function Jemput() {
  ...
  const completedCount = tasks.filter((item) => item.status === "Dijemput").length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: "Total Penjemputan", value: tasks.length, colors: "from-blue-50 to-blue-100 border-blue-200 text-blue-900" },
          { label: "Selesai", value: completedCount, colors: "from-green-50 to-green-100 border-green-200 text-green-900" },
          { label: "Sisa Tugas", value: tasks.length - completedCount, colors: "from-yellow-50 to-yellow-100 border-yellow-200 text-yellow-900" },
        ].map((card) => (
          <div key={card.label} className={`bg-linear-to-br ${card.colors} rounded-xl p-4 border hover:shadow-md transition-shadow`}>
            ...
            <p className={`text-2xl font-inter-semibold ${card.text || ""}`}>{card.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-md transition-shadow">
        <div className="px-6 py-5 border-b border-gray-100 bg-linear-to-r from-gray-50 to-white">
          <h2 className="font-inter-semibold text-lg text-gray-800">Jadwal Penjemputan Hari Ini</h2>
          <p className="text-sm text-gray-500 mt-1">{filtered.length} penjemputan ditampilkan</p>
        </div>
                  ...
                  <tr key={task.kode} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-center">
                      <button
                        ...
                        className="inline-flex items-center justify-center px-3 py-2 rounded-lg text-sm font-inter-medium bg-blue-50 text-[#3b6fd8] hover:bg-blue-100 transition-all group-hover:opacity-100"
                      >
                        Detail
                      </button>
                    </td>
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-gray-500 font-poppins">Tidak ada penjemputan yang cocok.</p>
                    </div>
                  </td>
                </tr>
              )}
          ...
          <div className="relative max-w-lg mx-auto mt-16 px-4">
            <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
              <div className="px-6 py-5 border-b bg-linear-to-r from-gray-50 to-white flex items-start justify-between">
                <div>
                  <h3 className="font-inter-semibold text-xl text-gray-800">Detail Penjemputan</h3>
