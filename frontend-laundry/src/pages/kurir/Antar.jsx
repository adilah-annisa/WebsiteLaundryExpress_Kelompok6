    ...
    <div className="w-full max-w-screen-xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="px-6 py-5 border-b flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h2 className="font-semibold text-lg">Jadwal Pengantaran</h2>
            <p className="text-sm text-gray-500">{tasks.length} tugas</p>
          </div>
          <Link to="/kurir/bukti" className="text-sm font-semibold text-[#1565C0] hover:underline">Upload Bukti →</Link>
        </div>

        <div className="px-6 py-4 border-b">
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Cari..." className="w-full px-4 py-2.5 border rounded-xl" />
        </div>

        {tasks.length === 0 ? (
          <p className="px-6 py-12 text-center text-gray-500">Belum ada tugas pengantaran.</p>
        ) : (
                ...
                {tasks.map((task) => (
                  <tr key={task.kode} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <button type="button" onClick={() => setSelected(task)} className="text-sm text-[#1565C0] font-semibold hover:underline">Detail</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="relative max-w-lg mx-auto mt-16 px-4">
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
              <div className="px-6 py-5 border-b flex justify-between">
                <div>
                  <h3 className="font-semibold text-xl">Detail Pengantaran</h3>
              ...
              <div className="p-6 space-y-4">
                <div className="flex items-center gap-3 p-4 bg-blue-50 rounded-xl">
                  ...
                  <div>
                    ...
                    <a href={`tel:${selected.telepon}`} className="text-sm text-blue-600 flex items-center gap-1 mt-1"><IoCallOutline /> {selected.telepon}</a>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {STATUS_FLOW.map((status) => (
                      <button key={status} type="button" disabled={selected.status === status} onClick={() => { setNewStatus(status); setConfirmOpen(true); }} className="px-4 py-2 rounded-xl border text-sm font-semibold disabled:opacity-40">
                        {status}
                      </button>
                    ))}
                  </div>
                  {selected.status === "Diantar" && (
                    <p className="text-xs text-blue-600 mt-2">Jangan lupa <Link to="/kurir/bukti" className="underline font-semibold">upload bukti pengantaran</Link>.</p>
                  )}
                </div>
              </div>
            </div>
